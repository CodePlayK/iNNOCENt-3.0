...
func _on_follow_timer_timeout() -> void:
	...
	if state_manager.current_state != self:
		return
	if npc.astar.only_update_on_floor and not PlayerState.is_player_on_floor():
		return
	var idle_range = npc.follwing_idel_range
	if idle_range and idle_range.has_overlapping_bodies():
		return
