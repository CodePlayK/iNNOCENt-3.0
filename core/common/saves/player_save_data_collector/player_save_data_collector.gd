...
## [b]挂载要求：[/b] 必须挂在目标储存对象相关节点下，且有且仅有一个 [BasePlayerSaver] 子节点。
...
func _setup_saver() -> void:
	saver = _find_saver()
	if not saver:
		push_error("%s: 未找到 BasePlayerSaver 子节点" % name)
		return
	saver.save_data_config = save_data_config
	saver.load_save.connect(_load_save)
	saver.load_save_file.connect(_load_save_file)


func _find_saver() -> BasePlayerSaver:
	for child in get_children():
		if child is BasePlayerSaver:
			return child
	return null
...
## 载入存档
func _load_save_file(data: Dictionary) -> void:
	if not enable_load:
		return
	load_load_save_file_custom_data(data)
...
## 子类重写：载入自定义数据
func load_load_save_file_custom_data(data: Dictionary) -> void:
	pass
