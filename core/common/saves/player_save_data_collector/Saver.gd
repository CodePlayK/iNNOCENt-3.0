extends Node
## 必须为 [PlayerDataCollector] 的唯一子节点。[br]
## 负责普通对象存档与读档的 IO，不负责数据组装与属性赋值。
class_name BasePlayerSaver

signal load_save(data: Dictionary)
signal load_save_file(data: Dictionary)

@onready var save_data_collector: PlayerDataCollector = get_parent() as PlayerDataCollector
...
func _connect_signals() -> void:
	...
	EventBus.player_load_save_file_pos.connect(_load_save_file)
...
func _load_save_file(level_id) -> void:
	save_data_collector.load_target_level = level_id
	if not save_data_collector or not save_data_collector.enable_load:
		return
	if not save_data_config:
		load_save_file.emit({})
...
## 响应 [signal EventBus.load_game] / Collector 主动调用
func _load_save_file_game() -> void:
	_load()
