...
@export_range(1.0,5.0) var vfx_bright:float:
	set(f):
		vfx_bright = f
		if !obj:return
		obj.base_fx.material.set_shader_parameter("mix_modulate_strength",vfx_bright)
var bright:float:
	set(f):
		bright = f
		if !obj:return
...
@export var vfx_color:Color:
	set(c):
		vfx_color = c
		if !obj:return
		obj.base_fx.material.set_shader_parameter("color",vfx_color)		
@export var vfx_colored:bool:
	set(f):
		vfx_colored = f
		if !obj:return
		obj.base_fx.material.set_shader_parameter("colored",f)		
...
func change_vfx_bright(bright,trans_time_vfx):
	var tween = obj.base_fx.create_tween()
