...
var node_type = 3
signal finished()
var condition_result_index:int = 0
...
func handle_condition(prop:Array):
	...
	if owner.print_debug:CutscenerGlobal.ACTION_LOG =" - 判断:[%s]--[%s]--[%s]" %[str(real_from_var),CutscenerGlobal.CONDITION_TYPE_DIC[int(condition_type)][0],str(real_to_var)]
