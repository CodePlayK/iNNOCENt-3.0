...
func on_refresh_setting_autoload_config() -> void:
	...
	# 不再在这里扫方法：由 WorkSpace / SettingDialog / 打开面板统一 request_reload_methods

func load_all_global(global_tree: Tree, list: Array) -> void:
	...
	for name in all_globals.keys():
		...
		item.set_text(2, str(all_globals.get(name, "")).replace("*res://", "res://"))
		if not get_tree().get_root().has_node(name):
			item.set_custom_color(0, Color(1, 0.6, 0.2))
...
func set_tree_selected(global_tree: Tree, list: Array) -> void:
