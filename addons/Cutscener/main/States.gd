...
func _unhandled_key_input(event: InputEvent) -> void:
	if !current_line_edit or !current_line_edit.has_focus():
		return
	if !event.is_action_pressed("ui_down") and !event.is_action_pressed("ui_up"):
		return
...
func on_load_global() -> void:
	...
	for bus in CutscenerGlobal.STATE_BUSES:
		if not get_tree().get_root().has_node(bus):
			continue
		var node: Node = get_tree().get_root().get_node(bus)
		for prop in node.get_property_list():
			if prop.usage & PROPERTY_USAGE_SCRIPT_VARIABLE:
				var pname: String = prop["name"]
				if pname not in state_list:
					state_list.append(pname)
					add_item(pname)
...
func _on_item_activated(index: int) -> void:
	refrsh_markers()
	if !side_bar.visible or !current_line_edit:
		return
